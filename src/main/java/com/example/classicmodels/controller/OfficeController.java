...
import com.example.classicmodels.repository.OfficeRepository;
...
public class OfficeController {
  ...
  public ResponseEntity<Office> getOfficeByOfficeNumber(@PathVariable("officeCode") String officeCode) {
    Office _office = officeRepository.findById(officeCode)
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Office with id = " + officeCode + " not found"));

    return new ResponseEntity<>(_office, HttpStatus.OK);
  }

  @PutMapping("/offices/{officeCode}")
  public ResponseEntity<Office> updateOffice(@PathVariable("officeCode") String officeCode,
      @Valid @RequestBody Office office) {
    Office _office = officeRepository.findById(officeCode)
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
            "Office with officeCode = " + officeCode + " not found"));
